...
def b2(scores:dict):
    ...
    '''
    ret = sorted([name for name, score in scores.items() if score > 50])
    return ret        
...
def q3(name_score_dict):
    ...
    for name,score in name_score_dict.items():
        ...
        res.append(s)
        res.sort(key=lambda s:s.score,reverse=True)
    return res
...
def q5():
    ...
    distance_count=x**2+y**2
    in_zone=np.sum(distance_count<=1)
    res = 4*(in_zone/n)
    return res
...
def q6(file_path):
    ...
    obj=Layout.build_layout(file_path)
    return obj
